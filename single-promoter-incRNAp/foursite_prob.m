function p_all = foursite_prob(energyi,mut)
...
for tt = 1:tspan
    p_on = transcription_rate_new(nbd,energyi,mut,TF_conc_t(tt));
...
for cc = 1:length(config_all)
    ...
    p_all = zeros(tspan,1);
    for rr = 1:tspan
        p_all(rr) = prob_per_config_new(nbd, config_all(cc,:), energyi,mut,TF_conc_t(rr));
    end
